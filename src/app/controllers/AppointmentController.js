...
class AppointmentController {
   ...
   async store(req, res) {
      ...
      // Checks if user try appoint with himself

      if (provider_id === req.userId) {
        return res.status(401).json({ error: 'User cant appoint with himself' });
      }
      ...
      return res.json({ id, date, provider_id });
   }
   ...
   async delete(req, res) {
      ...
      if (appointment.canceled_at !== null) {
         return res.status(401).json({
            error: "Appointment was already been cancelled"
         });
      }
